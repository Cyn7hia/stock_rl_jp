    ...
    def train(self, max_episode=1000000, max_path_length=200, verbose=0):
        ...
        for e in range(max_episode):
            ...
            for i, r in enumerate(zip(rewards, discounted_rewards_)):
                ...
                outputs_[i] = np.minimum(1, np.maximum(0, predicteds_[i] + (outputs_[i] - predicteds_[i]) * abs(
                    discounted_reward)))
...
if __name__ == "__main__":
    ...
    env = MarketEnv(dir_path="./data/", target_codes=codeMap.keys(), input_codes=[], start_date="2010-08-25",
